...
// ══════════════════════════════════════════════════════════════════
// SISTEMA DE AUTENTICACIÓN — Firebase Cloud Sync
//
// CONFIGURACIÓN OBLIGATORIA:
// Reemplaza los valores de firebaseConfig con los de tu proyecto de Firebase.
// Si no lo haces, el sistema usará localStorage como respaldo (modo offline).
// ══════════════════════════════════════════════════════════════════

const firebaseConfig = {
  apiKey: "AIzaSyCruqFTEq0EfmfiboG0vRZib1nRcBjxD5c",
  authDomain: "v-trading3.firebaseapp.com",
  projectId: "v-trading3",
  storageBucket: "v-trading3.firebasestorage.app",
  messagingSenderId: "880644869659",
  appId: "1:880644869659:web:346731e4d1164144f532be"
};
...
try {
  if (firebaseConfig.apiKey !== "TU_API_KEY") {
    ...
    console.warn("⚠️ Firebase no está configurado. Usando modo offline (localStorage).");
  }
} catch (e) {
...
async function syncUsersToCloud(usersArray) {
  if (!db) return;
  try {
    // Usamos un solo documento para guardar todo y no gastar cuota de lecturas/escrituras.
    await db.collection("vtrading").doc("users").set({ data: JSON.stringify(usersArray) });
  } catch (e) {
...
async function fetchUsersFromCloud() {
  if (!db) return null;
  try {
    const doc = await db.collection("vtrading").doc("users").get();
    if (doc.exists) {
...
async function syncDataToCloud(docName, dataArray) {
  ...
  try {
    ...
    console.error(`Error guardando ${docName} en la nube:`, e);
  }
}
...
async function fetchDataFromCloud(docName) {
  ...
  try {
    ...
    console.error(`Error leyendo ${docName} de la nube:`, e);
  }
  return null;
}
...
const MASTER_CREDENTIALS = {
  ...
  'ana@vtrading.com': {
    ...
    holdings: [],
    transactions: [{ type: 'dep', icon: '💳', name: 'Depósito Inicial', date: '20 feb, 10:00', amount: 23150.00 }]
  },
  'carlos@vtrading.com': {
    password: 'carlos1234',
    id: 3, name: 'Carlos López', role: 'user', idcard: 'VT-003',
    status: 'inactive', created: '2024-03-10',
    balance: 8900.75, balanceInvested: 0, todayProfit: -145.20,
...
// ── validateLogin: valida SIEMPRE contra MASTER_CREDENTIALS primero ──
// Si no está en master, busca en usuarios locales creados por el admin.
function validateLogin(email, password) {
  const emailNorm = email.trim().toLowerCase();
  const pwNorm = password.trim();

  // 1. Buscar en credenciales maestras (funciona en cualquier dispositivo)
  const masterEntry = Object.entries(MASTER_CREDENTIALS).find(
    ([e]) => e.toLowerCase() === emailNorm
  );
  if (masterEntry) {
    const [masterEmail, masterCreds] = masterEntry;

    // Verificar si el admin cambió la contraseña de este usuario maestro localmente
    const localUser = usersDB.find(u => u.email.toLowerCase() === emailNorm);
    const validPassword = localUser && localUser.password ? localUser.password : masterCreds.password;

    if (!comparePassword(pwNorm, validPassword)) return { error: 'wrong_password' };
    const validStatus = localUser && localUser.status ? localUser.status : masterCreds.status;
    if (validStatus === 'inactive') return { error: 'inactive' };

    // Devolver el usuario fusionado con datos locales de runtime
    const fullUser = localUser || getDefaults().find(u => u.email.toLowerCase() === emailNorm);
    return { user: fullUser };
  }

  // 2. Buscar en usuarios creados localmente por el admin
  const localUser = usersDB.find(u => u.email.toLowerCase() === emailNorm);
  if (!localUser) return { error: 'not_found' };

  if (!comparePassword(pwNorm, localUser.password)) return { error: 'wrong_password' };
  if (localUser.status === 'inactive') return { error: 'inactive' };
...
ge('security-form').addEventListener('submit', e => {
  e.preventDefault();
  ...
  ge('security-form').reset(); showToast('Contraseña actualizada ✓');
});
...
ge('user-form').addEventListener('submit', e => {
  e.preventDefault();
  ...
  saveUsers(usersDB); closeUserModal(); renderAdmin();
});
...
ge('confirm-delete').addEventListener('click', () => {
  if (!deleteId) return;
  ...
  saveUsers(usersDB); ge('confirm-modal').style.display = 'none';
  deleteId = null; renderAdmin(); showToast('Usuario eliminado ✓');
});
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Sincronizar en segundo plano con Firebase al cargar la página
  const cloudUsers = await fetchUsersFromCloud();
  if (cloudUsers) {
    localStorage.setItem('vt_users', JSON.stringify(cloudUsers));
    usersDB = loadUsers();
  }
  ...
  if (currentUser) {
    ...
    // Sincronizar currentUser con usersDB por si hubo cambios en otros lados
    const dbUser = usersDB.find(u => u.id === currentUser.id);
...
ge('login-form').addEventListener('submit', async e => {
  ...
  qs('.btn-text').style.display = 'none'; qs('.btn-loader').style.display = 'flex';
  ge('login-btn').disabled = true;

  // Sincronizar justo antes del login para tener la DB más fresca
  const cloudUsers = await fetchUsersFromCloud();
  if (cloudUsers) {
    localStorage.setItem('vt_users', JSON.stringify(cloudUsers));
  }
  ...
  setTimeout(() => {
    ...
    // ✅ Validación contra MASTER_CREDENTIALS y LocalStorage/Firebase
    ...
    qs('.btn-text').style.display = 'inline'; qs('.btn-loader').style.display = 'none';
